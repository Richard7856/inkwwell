...
**Risks/Limitations:** `autoVerify` falla en SILENCIO — si la verificación no pasa, el link simplemente abre el navegador, sin error ni log visible para el usuario. Por eso es tan común que "los App Links no funcionen" sin saber por qué.
**Improvement opportunities:** Resuelto abajo — ver la entrada de App Links verificados.

## [2026-08-09] App Links verificados contra inkwwell.vercel.app
**Context:** El intent-filter necesita el dominio real y `autoVerify` necesita que ese dominio sirva `/.well-known/assetlinks.json` con la huella SHA-256 del certificado que firmó el APK. Además había una duda concreta: `vercel.json` reescribe `/(.*)` a `/index.html` para el SPA, lo que podría tragarse la petición de `assetlinks.json` y devolverle HTML a Android.
**Decision:** Host `inkwwell.vercel.app` (el dominio de producción del proyecto en Vercel). El `assetlinks.json` vive en `public/.well-known/` y se genera con `npm run assetlinks`, que lee `android/keystore.properties`, saca la huella con `keytool` y la escribe. **No hubo que tocar `vercel.json`**: se verificó contra el sitio en vivo que Vercel resuelve el filesystem ANTES de los rewrites (`/favicon.svg` devuelve `image/svg+xml`, una ruta inexistente devuelve el `index.html`). También se confirmó que Vite copia directorios que empiezan con punto desde `public/` al build.
**Alternatives considered:** Escribir el JSON a mano (la huella tiene 64 caracteres hex en 32 pares — un typo la rompe y el fallo es silencioso). Agregar una excepción de rewrite en `vercel.json` (innecesaria, y una regla de más que confunde a quien la lea después).
**Risks/Limitations:** **El archivo solo existe para Android cuando la web está desplegada en producción.** Vive en la rama de trabajo; hasta que se mergee a `main` y Vercel despliegue, la verificación seguirá fallando. El script hace MERGE de huellas, no sobrescribe, justamente porque hará falta tener dos a la vez (ver abajo).
**Improvement opportunities:** Si se agrega un dominio propio, hay que duplicar el intent-filter con ese host y servir el mismo `assetlinks.json` desde ahí también.

## [2026-08-09] Firma de release vía keystore.properties (fuera del repo)
**Context:** Un APK de release sin firmar no se instala en Android ni lo acepta Play Store. Las credenciales de firma no pueden vivir en `build.gradle`, que sí se commitea.
**Decision:** `android/app/build.gradle` lee `android/keystore.properties` (en `.gitignore`) y arma el `signingConfig` de release. Si el archivo no existe, el build de release funciona igual pero produce un APK sin firmar — intencional, para que quien solo compile debug no necesite la llave. Hay un `keystore.properties.example` con la forma esperada y el comando de `keytool`.
**Alternatives considered:** Variables de entorno (funcionan en CI pero son incómodas en local y se pierden entre sesiones de terminal). Firmar a mano con `apksigner` después del build (un paso manual más que se olvida).
**Risks/Limitations:** **El keystore actual se generó dentro de una sesión de Claude Code, así que la llave y su contraseña pasaron por ese transcript.** Como la app aún no se publica, el riesgo real es nulo hoy — y con Play App Signing esta llave pasa a ser solo la de SUBIDA, que Google puede resetear. Aun así, lo correcto antes de publicar es regenerarla localmente con el comando del `.example` y volver a correr `npm run assetlinks`.
**Improvement opportunities:** Al publicar en Play con App Signing, Google RE-FIRMA el APK con su propia llave: la huella que importa para los App Links pasa a ser la de Play Console → Integridad de la app, NO la local. Por eso `npm run assetlinks -- --sha256 <huella>` agrega huellas en vez de reemplazarlas — hay que tener las dos, o los links funcionan en APK sideloaded pero no en el instalado desde Play (o al revés).
